    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.budgets.update", [$budget->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('budget_category') ? 'is-invalid' : '' }}" name="budget_category_id" id="budget_category_id">
                    @foreach($budget_categories as $id => $entry)
                        <option value="{{ $id }}" {{ (old('budget_category_id') ? old('budget_category_id') : $budget->budget_category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('start_date') ? 'is-invalid' : '' }}" type="text" name="start_date" id="start_date" value="{{ old('start_date', $budget->start_date) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('end_date') ? 'is-invalid' : '' }}" type="text" name="end_date" id="end_date" value="{{ old('end_date', $budget->end_date) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" type="number" name="amount" id="amount" value="{{ old('amount', $budget->amount) }}" step="0.01" min="1" required>
